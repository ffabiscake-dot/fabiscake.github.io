...
const CACHE_KEY = "fabiscake_products_cache_v5";
...
try {
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (key && key.startsWith("fabiscake_products_cache_") && key !== CACHE_KEY) {
...
onSnapshot(q, (snapshot) => {
    ...
    // Si Firestore tiene datos, actualiza el caché y re-renderiza
  ...
  // En caso de error, si hay datos en caché o data.js, intenta usarlos
  if (!hasFallback()) {
    console.warn("No hay productos disponibles. Verifica la conexión a Firestore.");
  }
});
